    ...
    <div className="modal fade-in" onClick={onCancel} style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, background: 'rgba(0,0,0,0.18)' }}>
      <div className="modal-content" onClick={e => e.stopPropagation()} style={{ minWidth: 340, maxWidth: '95vw', background: '#fff', borderRadius: 22, boxShadow: '0 8px 32px rgba(0,0,0,0.18)', padding: 0, overflow: 'hidden', position: 'relative', fontFamily: 'inherit' }}>
        ...
        {toast && toast.show && toast.message && toast.context === 'placeOrder' && (
          <div className="toast" style={{ margin: '18px 32px 0 32px', background: toast.type === 'error' ? '#d32f2f' : '#323232', color: '#fff', padding: '12px 32px', borderRadius: 10, fontWeight: 700, fontSize: '1.1rem', opacity: 0.97, textAlign: 'center', minWidth: 180, boxShadow: '0 2px 8px rgba(211,47,47,0.10)' }}>
            {toast.message}
          </div>
        )}
        <div className="dialog-body" style={{ padding: '28px 36px', background: '#f8f9fa' }}>
          ...
          <div style={{ marginTop: 22, background: '#fff', borderRadius: 12, boxShadow: '0 1px 6px rgba(0,0,0,0.04)', padding: '18px 20px' }}>
            ...
            <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <label htmlFor="po-name" style={{ fontWeight: 700, fontSize: '1rem' }}>Name</label>
              <input id="po-name" name="customerName" type="text" autoComplete="off" spellCheck={false} autoCapitalize="words" placeholder="Enter your name" aria-label="Name" value={localName} onChange={e => { setLocalName(e.target.value); setCustomerName(e.target.value); }} style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid #e6e6f0', fontSize: '1rem', marginTop: 2 }} />
            </div>
          ...
          <div style={{ display: 'flex', gap: 18, justifyContent: 'center', marginTop: 28 }}>
            ...
            <button onClick={() => onConfirm({ customer: localName, phone, notes: localNotes })} style={{ background: accentColor, color: '#fff', border: 'none', padding: '12px 28px', borderRadius: 10, fontWeight: 700, fontSize: '1.08rem', cursor: 'pointer', boxShadow: '0 2px 8px rgba(42, 92, 69, 0.10)' }}>Confirm Order</button>
          </div>
        </div>
      </div>
    </div>
